...
### Pro tip
Если непонятно с чего начать, стоит посмотреть на самый нижний уровень логики. В рамках этого упражнения стоит обратить внимание на класс `Receipt` и на метод `update_receipts` d `GildedRose`. Затем можно посмотреть сам генерацию отчётов в `generate_receipt_report`.
