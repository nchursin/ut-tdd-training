...
### Pro tip
Если непонятно с чего начать, стоит посмотреть на самый нижний уровень логики. В рамках этого упражнения стоит обратить внимание на класс `Receipt` и на метод `update_receipts` в `GildedRose`. Затем можно посмотреть сам генерацию отчётов в `generate_receipt_report`.
...
```bash
cd practice/04_gilded-rose-extended # если начинаете из корня репозитория
python3 -m venv .venv
